...
class NODE_OT_add_noodle_group(NodeAddOperator, Operator):
    '''Add a node to the active tree and link to an existing socket'''
    bl_idname = "node.add_"+APIPRE+"_group"
...
class NoodleMakeGroup(bpy.types.Operator):
    """Make newgroup node from selected nodes"""
    bl_idname = "node."+APIPRE+"_make_group"
    bl_label = "Make Group ("+NODETREE_EDITOR_NAME+")"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def poll(cls, context):
      ...
      if ntree.bl_idname != NODETREE_TYPE:
        return False
    ...
    def execute(self, context):
      ...
      ntree2 = bpy.data.node_groups.new('Group', NODETREE_TYPE)
      ...
      gn = ntree.nodes.new(PRE+'NodeGroup')
      gn.node_tree = ntree2
...
class NoodleEditGroup(bpy.types.Operator):
    """Make newgroup node from selected nodes"""
    bl_idname = "node."+APIPRE+"_edit_group"
    bl_label = "Edit Group ("+NODETREE_EDITOR_NAME+")"
